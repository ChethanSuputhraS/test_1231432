...
- (UIButton *)button
{
    if (!_button) {
        ...
        _button.imageView.layer.cornerRadius = 7.0f;
        _button.layer.shadowRadius = 3.0f;
        _button.layer.shadowColor = [UIColor blackColor].CGColor;
        _button.layer.shadowOffset = CGSizeMake(0.0f, 1.0f);
...
- (void)didTapButton:(id)sender
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"FloatButtonTapped" object:nil];
}
...
- (void)setupConstraints
{
    ...
    switch (_position) {
        ...
        case MEVFloatingButtonPositionBottomRight:
            _contentView.center = CGPointMake(self.frame.size.width - (_button.frame.size.width/2) + _horizontalOffset-30, self.frame.size.height - (_button.frame.size.height/2) + _verticalOffset);
            break;
...
- (void)mev_validateView
{
    if ([self mev_canDisplay]) {
        ...
        // Configure button
        if (view.image) {
            [view.button setFrame:CGRectMake(0, 0, view.image.size.width + view.imagePadding*2, view.image.size.width + view.imagePadding*2)];
            if (view.imageColor) {
