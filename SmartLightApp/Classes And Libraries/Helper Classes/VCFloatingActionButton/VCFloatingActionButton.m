...
@synthesize delegate,isAnimatedRequired;
...
-(id)initWithFrame:(CGRect)frame normalImage:(UIImage*)passiveImage andPressedImage:(UIImage*)activeImage withScrollview:(UIScrollView*)scrView
{
    ...
    if (self)
    {
        ...
        _menuTable.tableFooterView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH/2, CGRectGetHeight(frame))];

        _menuTable.delegate = self;
        _menuTable.dataSource = self;
        _menuTable.separatorStyle = UITableViewCellSeparatorStyleNone;
        _menuTable.backgroundColor = [UIColor clearColor]; // clear clor
        _menuTable.transform = CGAffineTransformMakeRotation(M_PI); // -mp//Rotate the table
...
-(void)setupButton
{
    ...
    self.backgroundColor = [UIColor clearColor]; // clear color
    ...
    _bgView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.0]; // alpha 0.8
    _bgView.alpha = 0;
    ...
    _bgView = vsview;
    _bgView.backgroundColor = UIColor.clearColor;// css add this
    [_bgView addGestureRecognizer:buttonTap2];
    
    _normalImageView = [[UIImageView alloc]initWithFrame:CGRectMake((self.frame.size.width-_normalImage.size.width)/2, (self.frame.size.width- _normalImage.size.height)/2, _normalImage.size.width, _normalImage.size.height)];
    _normalImageView.userInteractionEnabled = YES;
    _normalImageView.layer.shadowColor = [UIColor redColor].CGColor; // white color
    ...
    _pressedImageView.contentMode = UIViewContentModeScaleAspectFit; // css uncommected
...
-(void)ChangeImage:(UIImage *)normalImageName
{
    _normalImageView.frame = CGRectMake((self.frame.size.width-normalImageName.size.width)/2, (self.frame.size.width- normalImageName.size.height)/2, normalImageName.size.width, normalImageName.size.height);
    _normalImageView.image = normalImageName;
}
-(void)handleTap:(id)sender //Show Menu
{
    if (isAnimatedRequired == NO)
    {
...
-(void) showMenu:(id)sender  // chethan changed by 26th jan 2021 code to here
{
    if (isAnimatedRequired == YES)
    {
        ...
        [UIView animateWithDuration:animationTime/2 animations:^
         {
             self.bgView.alpha = 0.9;//1
             self.normalImageView.transform = CGAffineTransformMakeRotation(M_PI);// - mp is there
             self.normalImageView.alpha = 1; //0.7

             self.pressedImageView.transform = CGAffineTransformIdentity;
             self.pressedImageView.alpha = 1;//0.7
    ...
    {
        _pressedImageView.image = _normalImage;
        _normalImageView.image = _normalImage;
        ...
        [delegate didSelectMenuOptionAtIndex:0];
    }
}
        ...
        [UIView animateWithDuration:animationTime/2 animations:^
         {
             self.bgView.alpha = 0;
             self.pressedImageView.alpha = 0.f;
             self.pressedImageView.transform = CGAffineTransformMakeRotation(M_PI);// - mp is there
             self.normalImageView.transform = CGAffineTransformMakeRotation(0);
             self.normalImageView.alpha = 1.f;
         } completion:^(BOOL finished)
         {
