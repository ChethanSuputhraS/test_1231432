...
- (void) setupDefaultConfigurations
{
    ...
    self.sectorsRadius = 45.0 * approaxSize;
    self.backgroundColor = [UIColor clearColor];
    self.startAngle = toRadians(270*approaxSize);
...
//            NSLog(@"New Value=%f  and endVal=%f",newValue,trackingSector.endValue);

            double diff = trackingSector.endValue - newValue;
            if(diff > ((trackingSector.maxValue - trackingSector.minValue)/2))
            {
//                NSLog(@"New Value which is Undre=%f",newValue);
                trackingSector.endValue = trackingSector.maxValue;
                [self valueChangedNotification];
                [self setNeedsDisplay];
//                NSLog(@"FIRST New Value=%f  and endVal=%f",newValue,trackingSector.endValue);
            ...
            else
            {
                if (newValue <1)
                {
//                    NSLog(@"SECOND NO Value=%f  and endVal=%f",newValue,trackingSector.endValue);

                    return NO;
                }
//                NSLog(@"SECOND YES Value=%f  and endVal=%f",newValue,trackingSector.endValue);

            }
        }
        else
        {
//            NSLog(@"THIRD New Value=%f  and endVal=%f",newValue,trackingSector.endValue);
...
//    [UIFont boldSystemFontOfSize:drawInf.endMarkerFontize]
    //drawing end marker's text
    [self drawString:endMarkerStr
            withFont:[UIFont fontWithName:@"CenturyGothic-Bold" size:drawInf.endMarkerFontize]
...
- (SASectorDrawingInformation) sectorToDrawInf:(SAMultisectorSector *)sector position:(NSInteger)position
{
    ...
    if (position==3)
    {
        drawInf.radius = (self.sectorsRadius + 5) * position;
    }
    else if (position ==2)
    {
        drawInf.radius = (self.sectorsRadius + 15) * position;
    }
    else if (position == 1)
    {
        drawInf.radius = (self.sectorsRadius + 45) * position;
    }
    else
    {
        drawInf.radius = (self.sectorsRadius + 45) * position;
...
//    drawInf.radius = self.sectorsRadius * position;
    ...
    CGFloat minMarkerRadius = (self.sectorsRadius / 4.0)*approaxSize +1.5;
    CGFloat maxMarkerRadius = (self.sectorsRadius / 4.0)*approaxSize + 1.5;
    ...
    CGFloat minFontSize = 8.0 * approaxSize ;
    CGFloat maxFontSize = 10.0 * approaxSize;
...
//    font = [UIFont fontWithName:@"CenturyGothic" size:10];
    
//    NSLog(@"Color=%@ & Center.X=%f Center.Y=%f",color,x,y);
